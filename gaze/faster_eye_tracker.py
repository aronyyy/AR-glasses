...
def get_video_source():
    """Ask user whether to use webcam or upload a video."""
    root = tk.Tk()
    root.withdraw()  # Hide the main tkinter window

    choice = tk.messagebox.askquestion(
        "Video Source",
        "Do you want to use the webcam?\nClick 'No' to upload a video file."
    )
    ...
    if choice == 'yes':
        ...
        if not cap.isOpened():
            tk.messagebox.showerror("Error", "Webcam not found or cannot be opened.")
            sys.exit()
        print("Using webcam as source.")
        return cap
    else:
        ...
        if not file_path:
            tk.messagebox.showwarning("No file", "No file selected. Exiting...")
            sys.exit()
        cap = cv2.VideoCapture(file_path)
        if not cap.isOpened():
            tk.messagebox.showerror("Error", "Could not open the selected video file.")
            sys.exit()
        print(f"Using video file: {os.path.basename(file_path)}")
        return cap
...
if __name__ == "__main__":
    # If you want to skip the GUI and go straight to the camera:
    if GL_SPHERE_AVAILABLE:
